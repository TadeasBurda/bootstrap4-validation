...
//    Daleje je nutne pridaù data-validation-group="{{n·zov gr˙py}}" aspon pre jeden <button></button>.
//    Do <body></body> pridaj <div data-validation-for="{{id validovanÈho inputu}}"></div> a v tvojom main.js s˙bore, 
//    vytvor instanciu Validation.ts(staËÌ jedena na cel˙ page) a zavolaj initialize().

//  Ako to funguje:
//    Po spusteni initialize() sa skontroluje ci na stranke existuje aspon jeden element z atributon data-validation="true".
//    Ak ·no, tak sa skontroluje validita nastavenÌ(kaûd˝ input musi byt v grupe z aspon jednim buttnom) a ak je vsetko v poriadku,
...
//    a podla vysledku sa zobrazÌ v tele DIVu pre feedback odpovedajuca hl·öka(dodana prehliadacom) + button grupy sa bud zablokuje alebo sprÌstupn pre akciu(disabled).
